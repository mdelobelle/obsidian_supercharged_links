...
import {
  SuperchargedLinksSettings,
  DEFAULT_SETTINGS,
} from "src/settings/SuperchargedLinksSettings";
...
export default class SuperchargedLinks extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    // Initialization
    this.registerEvent(
      this.app.workspace.on("window-open", (window, win) =>
        this.initModalObservers(this, window.getContainer().doc),
      ),
    );
    ...
    this.registerEvent(
      this.app.metadataCache.on("changed", debounce(updateLinks, 500, true)),
    );
  ...
  initViewObservers(plugin: SuperchargedLinks) {
    ...
    if (plugin.app?.plugins?.plugins?.breadcrumbs) {
      ...
      plugin.registerViewType(
        "markdown",
        plugin,
        ".BC-page-views span.internal-link, .BC-codeblock-tree span.internal-link, .nodes a.internal-link",
      );
    }
    ...
    // @ts-ignore
    if (
      (plugin.app as any)?.internalPlugins?.plugins?.bases?.enabled &&
      ...
      plugin.registerViewType(
        "markdown",
        plugin,
        "div.bases-table-cell span.internal-link",
      );
      plugin.registerViewType(
        "markdown",
        plugin,
        "div.bases-table-cell div.multi-select-pill-content",
      );
    ...
    // @ts-ignore
    if (
      (plugin.app as any)?.internalPlugins?.plugins?.backlink?.enabled &&
      (plugin.app as any)?.internalPlugins?.plugins?.backlink?.instance?.options
        ?.backlinkInDocument
    ) {
    ...
    plugin.registerViewType(
      "file-properties",
      plugin,
      "div.internal-link > .multi-select-pill-content",
    );
    if (plugin.app?.plugins?.plugins?.["notebook-navigator"]) {
      plugin.registerViewType(
        "notebook-navigator",
        plugin,
        "span.nn-shortcut-label",
      );
          ...
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach((n) => {
              if (
                ...
                let selector =
                  ".suggestion-title, .suggestion-note, .another-quick-switcher__item__title, .omnisearch-result__title > span";
                // @ts-ignore
                if (n.className.includes("suggestion-container")) {
                  selector = ".suggestion-title, .suggestion-note";
                }
                plugin.updateContainer(n as HTMLElement, plugin, selector);
                plugin._watchContainer(
                  null,
                  n as HTMLElement,
                  plugin,
                  selector,
  ...
  updateContainer(
    ...
    if (
      !plugin.settings.enableBacklinks &&
      container.getAttribute("data-type") !== "file-explorer"
    )
      return;
    if (
      !plugin.settings.enableFileList &&
      container.getAttribute("data-type") === "file-explorer"
    )
      return;
    ...
    let observer = new MutationObserver((records, _) => {
      records.forEach((mutation) => {
        if (mutation.type === "childList") {
          mutation.addedNodes.forEach((n) => {
            if ("className" in n) {
              const cls = (n as any).className as unknown;
